...
- `lo_rate_limit_kops`：高优先级压力窗口内低优先级的保护速率。数据面未指定环境变量时默认约为 `160 kops/s`。
...
PF-3 一键运行会保持验收阈值不变，仍按 `gain_pct >= 22%` 判定。为了避免本轮高低优先级差异过大，脚本使用 `THREADS=4`，并在测试阶段临时设置 `NR_LO_RATE_KOPS=150`，让低优先级保护略强于数据面默认值；测试结束后恢复数据面默认环境，清除 `NR_LO_RATE_KOPS`，回到未设置时的 `160 kops/s` 默认策略。
