(function () {
  ...
  function fallbackImplementation(detail) {
    ...
    if (calls.length) {
      ...
      return `页面按钮通过 Flask 功能验收接口触发该功能点脚本，脚本调用真实数据面接口完成验收。\n- 关键调用：${uniq.join("、")}`;
    }
    return "页面按钮通过 Flask 功能验收接口触发该功能点脚本，脚本按功能验收口径调用真实数据面或控制面接口，并写入可复查结果。";
  }
  ...
  function renderRequirement(detail) {
    ...
    const goal = raw.description || firstSection(md, ["功能点", "功能需求"]) || "见结果摘要";
